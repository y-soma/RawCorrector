...
public:
	...
	/*****************************************************************************\
		...
		[Purpose]
			コントロールへテキストを書き込む
	\*****************************************************************************/
	inline void SetWindowTxt(const TCHAR* const str)
	{
		SetWindowText(m_hDlg, (LPTSTR)str);
	}
	...
	/*****************************************************************************\
		...
		[Returns]
			表示されているテキストが文字列で返る
	\*****************************************************************************/
	inline String GetWindowTxt(void)
	{
		TCHAR str[MAX_PATH*0xFF];
		GetWindowText(m_hDlg, str, MAX_PATH * 0xFF);
		return str;
	}
...
private:
	/*----------------------------------------------------------------------------\
		...
		[Parameter1] wParam
			wParam
	\----------------------------------------------------------------------------*/
	void KeyAccelerator(const WPARAM& wParam)
	{
		switch(LOWORD(wParam))
		{
		case VK_SPACE:
