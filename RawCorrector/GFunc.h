...
public:
	/*****************************************************************************\
		...
		[Returns]
			成功すると分割された文字列が配列で返る
	\*****************************************************************************/
	static std::vector<String> Split(const TCHAR* const str, const TCHAR* const delim)
	{
		std::vector<String> items(0,_T(""));
		String src = str;
	...
	**************************************************************************/
	...
	/**************************************************************************
		[Function]
			GetIniAllSection
		[Purpose]
			Initファイルの一つのセクションの全てのパラメータを取得する
		[Parameter1] filename
			iniファイル名
		[Parameter2] section
			セクション名
		[Parameter4] specdir
			実行ファイル以外の場所からiniファイルを読み込む場合はここでディレクトリを指定(※ディレクトリの最後に"\"を付ける)
		[Returns]
			成功するとセクション内の全てのパラメータを文字列の配列で返却する
			一つの配列につき、"キー=パラメータ値" の形式で格納する
	**************************************************************************/
	static std::vector<String> GetIniAllSection(const TCHAR* const filename, const TCHAR* const section, const TCHAR* const specdir = NULL)
	{
		std::vector<String> dst(0, _T(""));
		try
		{
			...
			const String path = dir + filename;
			const unsigned long buf_size = MAX_PATH * 0xFF;
			TCHAR prmbuf[buf_size] = {0};
			GetPrivateProfileSection(section, prmbuf, buf_size, path.data());
			
			{// 読み込んだ文字列を適切に分割し、配列へ格納する
				String str_tmp = _T("");
				int null_cnt = 0;
				TCHAR end_str = 0, comment_str1 = 0, comment_str2 = 0;
				#ifdef UNICODE
					end_str = L'\0';
					comment_str1 = L'#';
					comment_str2 = L'/';
				...
				{// 文字列中に"00"が来るまで読み込む
					...
					else if(prmbuf[i] == end_str)
					{
						++null_cnt;
						if(null_cnt > 1) break;
						const TCHAR* tstr = str_tmp.c_str();
						// 先頭に'#'と'/'があればコメントとし、読み飛ばす
						if(tstr[0] != comment_str1 && tstr[0] != comment_str2)
							dst.push_back(str_tmp);
						str_tmp = _T("");
					}
				}
			}
