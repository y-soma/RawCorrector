...
protected:
	...
	virtual BOOL OnInitDialog(const HWND hWnd, const HWND hwndFocus, const LPARAM lParam)
	{
		...
		// 初期値挿入
		SetWindowText(m_hEditWidth, m_Inputdatainfo.width);
		SetWindowText(m_hEditHeight, m_Inputdatainfo.height);
		SetWindowText(m_hEditBit, m_Inputdatainfo.bit);
		SetWindowText(m_hEditHeader, m_Inputdatainfo.headersize);
		SetCheckStartcolor(GetStartcolorInt(m_Inputdatainfo.startcolor));
		SetCheckDatavector(GetDatavectorInt(m_Inputdatainfo.datavector));
		SetCheckEndian(GetEndianInt(m_Inputdatainfo.endian));
	...
	virtual LRESULT CommandProc(const WPARAM wParam)
	{
		...
		{
		case IDOK:{// OK
			m_Inputdatainfo.startcolor = GetStartcolorStr(GetCheckStartcolor());
			m_Inputdatainfo.datavector = GetDatavectorStr(GetCheckDatavector());
			m_Inputdatainfo.endian = GetEndianStr(GetCheckEndian());
			GetWindowText(m_hEditWidth, (LPTSTR)m_Inputdatainfo.width, MAX_PATH);
			GetWindowText(m_hEditHeight, (LPTSTR)m_Inputdatainfo.height, MAX_PATH);
			GetWindowText(m_hEditBit, (LPTSTR)m_Inputdatainfo.bit, MAX_PATH);
			GetWindowText(m_hEditHeader, (LPTSTR)m_Inputdatainfo.headersize, MAX_PATH);
			EndDialog(m_hDlg, IDOK);
			break;}
		case IDCANCEL:{// 終了
			EndDialog(m_hDlg, IDCANCEL);
